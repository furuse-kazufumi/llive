...
| MATH-03 | Phase 10 | Pending (LaTeX/MathML 構文解析、実 Brief で要求が surface してから着手) | MED |
| MATH-04 | Phase 10 | Pending (IEEE 754 精度トラッキング、観察で必要性が出てから) | MED |
| MATH-06 | Phase 10 | **Implemented (minimal scale layer)** (2026-05-17, `unit_scale_factor()` + `UnitCitation.si_factor/si_value`、composite (km/h) 対応。Quantity API は破壊変更回避のため触らず citation 層のみ scale 化。Buckingham π 無次元化は後段) | MED |
| MATH-07 | Phase 10 | **Implemented** (2026-05-17, `EpistemicType.MATHEMATICAL` + `mathematical_track` confidence ≥ 0.8 strict mode + MathVerifier 検算 hint) | MED |
