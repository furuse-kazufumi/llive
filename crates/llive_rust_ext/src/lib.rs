...
/// RUST-17b novelty_score_batch — k-NN L2 mean for each query vs archive.
///
/// Improvements over RUST-17 baseline:
///   * **rayon par_iter** on N (集団 N=64 を CPU cores で並列化).
///   * **select_nth_unstable** (Hoare quickselect, O(A) avg) for top-k —
///     replaces O(A log A) full sort.
///   * GIL released via `py.allow_threads` during the compute kernel so
///     concurrent Python work isn't blocked.
///
...
fn novelty_score_batch(
    ...
    // Release GIL while we crunch — rayon par_iter on N, quickselect on A.
    let result: Vec<f64> = py.allow_threads(|| {
        (0..n)
            .into_par_iter()
            .map(|i| {
                let mut dists: Vec<f64> = Vec::with_capacity(a_n);
                for j in 0..a_n {
                    let mut sum_sq = 0.0_f64;
                    for d in 0..dim {
                ...
                // Hoare quickselect: partial sort so the first k_use entries are
                // the k smallest (any order). Mean is order-invariant.
                if k_use < dists.len() {
                    dists.select_nth_unstable_by(k_use, |x, y| {
                        x.partial_cmp(y).unwrap_or(std::cmp::Ordering::Equal)
                    });
                }
                dists[..k_use].iter().sum::<f64>() / k_use as f64
            })
            .collect()
    });
    Ok(result)
