...
PERSONA_ONTOLOGY: dict[str, Persona] = {
    ...
    # --- 拡張歴史人物 (2026-05-24 追加) -------------------------------------------
    # llive 文脈 (派生集団進化 / 予測符号化 / TRIZ / honest disclosure) で価値の高い
    # 思考の型を補う。factor_affinity は thought_patterns からの heuristic (CE-23 で精緻化)。
    # THOUGHT_FACTORS 順: structurize, recompose, closed_loop, self_extend, uncertainty,
    ...
    "darwin": Persona(
        ...
        thought_patterns=("自然選択", "変異と適応", "系統樹", "漸進", "膨大な観察"),
        # llive 派生集団進化の理論的祖. exploration / reality_link / self_extend 高.
        factor_affinity=(0.7, 0.8, 0.7, 0.85, 0.7, 0.9, 0.75, 0.8, 0.75, 0.95),
    ),
    "altshuller": Persona(
        persona_id="altshuller",
        name="ゲンリッヒ・アルトシュラー",
        era="20C-Inventive-Methodology",
        fields=("engineering", "inventive-problem-solving", "triz"),
        thought_patterns=("矛盾", "40の発明原理", "理想性", "資源", "進化のパターン"),
        # llive TRIZ 思考因子の祖. structurize / recompose / multiview 高.
        factor_affinity=(0.95, 0.9, 0.7, 0.7, 0.5, 0.7, 0.85, 0.6, 0.85, 0.8),
    ),
    "helmholtz": Persona(
        persona_id="helmholtz",
        ...
        # 予測符号化 (Friston) の源流. structurize / uncertainty / reality_link 高.
        factor_affinity=(0.85, 0.6, 0.75, 0.6, 0.8, 0.6, 0.85, 0.6, 0.8, 0.9),
    ),
    "shannon": Persona(
        persona_id="shannon",
        name="クロード・シャノン",
        ...
        # 予測誤差=情報 の基礎. structurize / uncertainty / consistency 高.
        factor_affinity=(0.9, 0.7, 0.6, 0.6, 0.9, 0.6, 0.9, 0.5, 0.6, 0.8),
    ),
    "turing": Persona(
        persona_id="turing",
        name="アラン・チューリング",
        ...
        # 計算/AI の祖. structurize / self_extend / exploration 高.
        factor_affinity=(0.9, 0.75, 0.6, 0.9, 0.6, 0.85, 0.85, 0.5, 0.8, 0.7),
    ),
    "poincare": Persona(
        persona_id="poincare",
        name="アンリ・ポアンカレ",
        ...
        # 岡潔と親和 (無意識の創造). recompose / exploration / multiview 高.
        factor_affinity=(0.8, 0.9, 0.5, 0.7, 0.7, 0.9, 0.7, 0.5, 0.9, 0.6),
    ),
    "kahneman": Persona(
        persona_id="kahneman",
        name="ダニエル・カーネマン",
        ...
        # honest disclosure (Millidge) と親和. uncertainty / multiview / reality_link 高.
        factor_affinity=(0.7, 0.5, 0.6, 0.3, 0.95, 0.5, 0.7, 0.6, 0.9, 0.85),
    ),
}
