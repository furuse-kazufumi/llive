...
## 実装状況 (2026-05-16 時点、llmesh v3.1.0 で確認)

llmesh repo の `llmesh/discovery/` を点検した結果、**Phase 1 / Phase 2 部分 /
Phase 4 は既に実装済** であることが判明。本 RFC で「これから実装する」と
書く部分は実質 Phase 3 / 5 / 6 に絞られる。

| Module (existing)                        | 行数 | 担当する機能 | RFC Phase |
|------------------------------------------|------|--------------|-----------|
| `llmesh/discovery/dns_sd.py`             | 208  | mDNS publication + DID + capability_hash + data_levels TXT | **Phase 1 ✓** |
| `llmesh/discovery/registry.py`           | 196  | NodeRegistry (in-memory, TTL eviction, subnet filter, signature verify) | **Phase 1 ✓ / Phase 2 ◐** |
| `llmesh/discovery/router.py`             | 119  | FastAPI /registry/{register,nodes} endpoints | Phase 1 ✓ |
| `llmesh/discovery/client.py`             | 173  | DiscoveryClient (stdlib urllib) | Phase 1 ✓ |
| `llmesh/discovery/gossip.py`             | 162  | Periodic /registry/peers exchange | **Phase 4 ✓** |
| `llmesh/discovery/encrypted_announce.py` | 227  | Ed25519 signed announce + Phase 2 encryption | Phase 2 ◐ (signature 済 / Onion は未) |

凡例: ✓ 実装済 / ◐ 部分実装 / ☐ 未実装

## 既存資産との関係

| 層                    | 既存 (llmesh v3.1.0)              | 本 RFC で拡張する部分                |
|-----------------------|-----------------------------------|--------------------------------------|
| Hub topology          | mesh-of-hubs 基盤あり (NodeRegistry) | DHT 越しの広域 mesh                  |
| Discovery             | **mDNS LAN ✓**, static config     | DHT (Kademlia) 広域、capability clustering 強化 |
| Routing               | direct hub → client               | overlay routing (中継 1〜3 hop)      |
| Privacy               | Ed25519 signed announce、TLS、audit chain | onion routing (optional)        |
| Knowledge sharing     | manual import_rad.py              | skill chunk replication (DTKR 連動)  |
| Learning              | (none)                            | local learning rules (EDLA 候補)     |
| Coordination          | **gossip protocol ✓ /registry/peers** | capability-aware partitioning   |
