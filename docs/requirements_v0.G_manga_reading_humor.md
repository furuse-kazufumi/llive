...
### 柱 C-Plus: 日本語多義性 (Polysemy) と「スナックバス江」型ユーモア

ユーザー追加指摘 (2026-05-22):

> 漫画は色々ありますが、私は「スナックバス江」とかが好きですね。日本語の多義的な妙味を感じさせてくれます。

「スナックバス江」(フォビドゥン澁川 / 講談社) — シニカルな間 / 説明過多にしない文体 / 何気ない会話に多義性が宿るタイプの作品. これを reference 軸として:

#### CP-1. 多義性 (Polysemy) 検出

- 同音異義 / 同綴異義 / 文脈で意味が反転する語の検出
- 「で?」「いや」「あー」など **意味が文脈で十重二十重に変わる短語** の追跡
- 助詞一文字の質感差 (「は」 vs 「が」 vs 「も」) も対象

#### CP-2. シニカル / 黙りオチ / 余韻型構造

- 説明過多にしない: panel transition の **「省略」「無言の panel」「視線だけの panel」** をユーモア構造として記録
- HumorKB category に `黙りオチ` / `視点だけ panel` / `間で笑わせる` を追加

#### CP-3. 一次資料の扱い

- スナックバス江は **著作権ありの商業作品**. 柱 D に従い:
  - 直接スキャンしない / 直接引用しない
  - 「該当作品から学ばれたパターンのうち、抽象化された構造のみ」を HumorKB に
  - ユーザー所有物の合法購入を前提とする

#### CP-4. 公式公開素材の活用 (2026-05-22 追記)

ユーザー指摘: 公式特設サイト `https://youngjump.jp/info/bazue/` が利用可能.

公式が **権利者として公開している** 範囲:

- 公式特設サイトの **紹介テキスト / キャラ紹介 / あらすじ** — 引用ガイドライン内で参照可
- 公式 trailer / PR 映像 — Vision encoder で構造分析可 (キャラ表情, panel 例)
- 出版社が **無料公開している試し読みエピソード** — 引用ガイドライン内で構造分析可

**運用**:

- 公式公開素材を **第一の reference 源** とし、ユーザー私物作品の参照は次点
- HumorKB の `source` field に出典 (公式 URL / ISBN + 公式公開分かどうか) を必ず記録
- 出版社の publish policy に変更があれば即時 refresh (年次)
- 公式公開素材であっても、HumorKB には **抽象構造のみ** (paraphrase) を保存 (柱 C-1, C-2 と同じ規律)

### 柱 C-Meme: 定着済ネットミームの取り込み

ユーザー追加指摘 (2026-05-22):

> よく使われるネットミームくらいは使えるようになりましょう。

[[feedback_article_humor_style]] の禁止項目 (架空対話 / 落語枕 / 過剰オノマトペ / AI 擬人化) を維持しつつ、**定着度が高く現代日本語話者の共通認識** になっているネットミームは利用可とする.

#### CM-1. 採用基準

- **定着度** ≥ 3 年以上の継続使用実績 + Twitter/X / 5ch / 増田 / Reddit 日本系で広く流通
- **多義性** = 単語自体に意味があり、文脈で意味が変わるもの (草 / ???/ 真顔 / なるほど 等)
- **作者特定が困難** = 出典が広く拡散済 (特定作家のキャラ口癖はミーム化していても NG)
- **時事性 = なし** = 流行半年で消えるネタは除外 (年代タグで管理)

#### CM-2. 採用候補 (例 — 月次見直し)

- `草` (笑い) / `???` (理解不能) / `(察し)` (省略) / `真顔` / `ンン!?` / `わかる` / `これすき` / `ガチで〜` / `ワロタ` / `クッソ〜` / `〜ニキ` (ただし職業/分野系のみ) / `結局〜` / `〜じゃん` (関西外を含む共通語化したもの) / `〜してくれ` (依頼の柔らかい命令)
- これらは **「広告コピー的に使える定型」** として HumorKB に登録

#### CM-3. 除外候補 (例)

- ❌ 特定作家のキャラ口癖 / 特定漫画 panel のミーム化フレーズ — 著作権 + 内輪化
- ❌ 流行半年のショート動画ミーム (年代タグで自動 expire)
- ❌ AI 擬人化に流れるミーム ([[feedback_article_humor_style]] NG)
- ❌ 過剰なオノマトペ ([[feedback_article_humor_style]] NG)

#### CM-4. 使用回数制限

- 1 記事あたり **2-3 個まで** ([[feedback_article_humor_style]] 「小さなユーモア (1 記事 1〜2 個まで)」を緩やかに拡張)
- 連続使用 (隣接段落での重複) 禁止
- ベンチ数値の honest disclosure 直前など **段落の落ち付近で限定的に使用** を推奨
