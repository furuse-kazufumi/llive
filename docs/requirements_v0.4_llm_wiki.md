...
- ✅ LLW-AC-10 (dynamic edge weight updates)
- ✅ LLW-AC-11 (exploration vs exploitation balance)

LLW-AC-02 / 06 / 07 は Phase 3-4 に倒すが、**設計時点で hooks を残しておく**。

### LLW-AC-11: Exploration vs Exploitation (Phase 2 必須)

**完全な suppression は dead-branch 問題を生む**。AC-10 で「読まれない edge は減衰 →
pruning」を入れたが、それだけだと「一度低くなった edge は二度と参照されない」局所最適に
陥る。AC-11 は **適度な循環を許容しつつ多様性を保つ** 4 つの仕組み：

#### A. Weight Floor (重み下限)

通常 pruning は `w < min_weight_keep (0.05)` で物理削除する。一方 AC-11 では：
- **`floor_weight = 0.02`** を導入。`min_weight_keep > w >= floor_weight` の edge は **削除せず
  dormant 扱い** とする（visit_count を 0 のままにし、ランダム覚醒対象として保持）
- dormant edge は通常 query (weight DESC) から外れるが、ランダム覚醒のときに復活できる

#### B. Random Resurrection (ランダム覚醒)

`EdgeWeightUpdater.random_boost(probability=0.05, boost=0.05)` を cron で定期実行：
- 全 edge (dormant 含む) を走査
- 確率 `p` (default 0.05) で boost 加算
- 結果 weight が `min_weight_keep` を超えたら通常 edge に復帰
- これにより「忘れられた」つながりが偶発的に再注目される

#### C. UCB-style Exploration Score

`EdgeWeightUpdater.exploration_score(weight, visit_count, total_visits, c=1.0)` を新規 API として
追加：
- effective score = `w + c * sqrt(ln(total_visits + 1) / (visit_count + 1))`
- 訪問回数の少ない edge ほど exploration term が大きく、上位に来やすい
- Wiki Compiler が「近隣」を LLM prompt に流す際は **effective_score で sort** する設計
  (Phase 2 では `query_neighbors` も残してデフォルト動作は AC-09 weight DESC)

#### D. Visit Tracking

各 edge の `visit_count` を tracking する。Phase 2 では in-memory dict
(`EdgeWeightUpdater._visit_counts`) で OK、Phase 4 で MemoryEdge schema 拡張時に永続化。
`on_read_hit` 時に visit_count をインクリメント。

#### 設計の意図

- AC-10 の決定論的減衰 + AC-11 の確率的覚醒で、**「収束させすぎず、しかし暴走もさせない」**
- ε-greedy や Thompson sampling と類似の構造、強化学習の lessons learned を流用
- HITL から見ると Wiki が「呼吸している」（活発な領域と休眠領域が時間で入れ替わる）
- AC-04 diversity preservation + AC-11 random resurrection の **両輪**で循環をコントロール

### LLW-AC-10: Dynamic Edge Weight (Phase 2 必須)

LLW-AC-09 で「edge weight に semantics がある」状態を作ったが、**weight は静的に set/upsert
されるだけでは循環防止として弱い**。読まれる edge が強化され、読まれない edge が減衰する
**動的更新サイクル**を必須化する。

**5 つの更新トリガー**：

| トリガー | 更新規則 | 適用 rel_type |
|---|---|---|
| **read_hit** (memory_read で参照された) | `w' = clip(w + α_read, 0, 1)`、default `α_read = 0.02` | linked_concept / co_occurs_with |
| **consolidation_recompute** (cycle 内で再計算) | Jaccard / count から重み再導出 (LLW-AC-09 table) | linked_concept / co_occurs_with |
| **time_decay** (cron で半減期適用) | `w' = w * exp(-Δt / τ)`、default τ = 30 日 (linked_concept) / 7 日 (co_occurs_with) | linked_concept / co_occurs_with / temporal_after |
| **contradiction_penalty** (diversity check で merge 拒否) | `w' = max(0, w - α_penalty)`、default `α_penalty = 0.1` | linked_concept |
| **surprise_boost** (新規 entry の追加で覚醒) | `w' = clip(w + α_surprise * surprise, 0, 1)` | linked_concept (該当 page を含む) |

**実装**：`llive.memory.edge_weight.EdgeWeightUpdater` を新規追加。

- `on_read_hit(page_id, neighbor_ids)` — `memory_read` ヒット時に呼ぶ
- `on_consolidation(touched_pages)` — Consolidator が cycle 後に呼ぶ
- `apply_time_decay(now=None)` — cron で daily 実行
- `on_contradiction(src_id, dst_id)` — diversity check が merge を拒否した時
- `on_surprise(page_id, surprise)` — surprise gate が write を通した時

**pruning**: 各更新後、`w < min_weight_keep` (default 0.05) の edge は **物理削除**。Wiki が
sparse に保たれ、無意味な弱リンクが思考循環を起こさないようにする。

**永続化**: 各 edge の `last_updated_at` を MemoryEdge に追加 (Phase 2 で schema 拡張可)。
動的更新の trace は `D:/data/llive/logs/edge_weight.jsonl` に append-only で記録 (HITL から
確認可能、Phase 4 で llove dashboard 化)。

**循環防止との連動**：

- 読まれない edge は減衰 → ConceptPage が「死んだ参照」を抱え続けない
- 矛盾を起こす edge は penalty → 偽の合意形成を阻止
- 新規 surprise が来ると関連 edge が boost → 静的固定化を防ぐ
- pruning により Wiki がコンパクトに保たれ、LLM context への押し込みも効率化

**Phase 2 で必須**: read_hit / consolidation_recompute / time_decay / pruning
**Phase 3 拡張**: contradiction_penalty / surprise_boost / per-page τ 動的調整
